...
// Sections (in order):
//   1. Cons// NarrativeOS v20 — Clean rebuild
// Sections (in order):
//   1. Constants & Config
...
function ResumeTab({ jd, setJd, resumeOnly, onDownloaded, profile }) {
  ...
  // Build a profile summary from available fields for prompt context
  const profileContext = [
    profile.title && `Title: ${profile.title}`,
    profile.background && `Background: ${profile.background}`,
    resume && `Resume (first 800 chars): ${resume.slice(0, 800)}`,
  ].filter(Boolean).join("\n");
  ...
  const run = async () => {
    ...
    try {
      const system = `You are a senior executive resume strategist for ${profile.name || "this candidate"}.
${profileContext}
...
function CoverLetterTab({ jd, setJd, onDownloaded, profile }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        if (m) {
          const p = JSON.parse(m[0]);
          if (p.company && !company) setCompany(p.company);
  ...
  // Build profile context from available fields
  const profileContext = [
    profile.title && `Title: ${profile.title}`,
    profile.background && `Background: ${profile.background}`,
  ...
  const run = async () => {
    ...
    setLoading(true); setError(null); setResult("");
    try {
      const system = `You are writing a cover letter for ${profile.name || "this candidate"}.
${profileContext}
...
function InterviewPrepTab({ jd, setJd, stories, profile }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const system = `You are an executive interview coach preparing ${profile.name || "this candidate"} for a ${round} interview.
${profileCtx}
Stories: ${storyCtx}
              ...
              <div style={{ padding: "24px", maxWidth: "860px" }}>
                ...
                <AnalyzeTab
                  ...
                  onBuildResume={() => {
                    ...
                    _pendingCard.current = newCard;
                    // Signal workspace to open on Resume tab
